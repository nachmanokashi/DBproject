...
![Integration Diagram](./שלב%203/תמונות/צילום%20מסך%202026-04-15%20163841.png)
_תרשים האינטגרציה המציג את ה"גשר" הלוגי המבוזר בין המערכות._
...
🔗 **[לחץ כאן לצפייה בקוד ה-SQL המלא של טבלת הגישור והגדרות ה-FDW](./שלב%203/Programing/bridge_setup.sql)**
...
🔗 **[לחץ כאן לצפייה בקוד ה-SQL המלא של המבטים](./שלב%203/Programing/views_creation.sql)**

---

### ⚡ יכולות אנליטיות מתקדמות (Queries on Views)

כדי להוכיח את יעילות הארכיטקטורה המבוזרת, ביצענו 5 שאילתות מורכבות ומתוחכמות ישירות על המבטים שיצרנו. השאילתות מדגימות ניתוח נתונים מתקדם המשלב פונקציות חלון (Window Functions) ואגרגציות מותנות:

#### 1. ניתוח סדרות עיתיות (Time-Series Analysis)

- **מטרה:** ניתוח מגמות של עומסים לוגיסטיים וקליניים לאורך זמן כדי לזהות זמני שיא (Peak Hours) בהגעת פצועים ומטופלים.
- **טכניקה:** שימוש בפונקציות חלון כגון `LAG` ו-`LEAD` למדידת קצב השינוי השעתי בין קבוצות מטופלים.
- **צילום הרצה ותוצאה:**
  ![Time Series Run](./שלב%203/תמונות/שאילתות/time_series_query.png)

#### 2. אגרגציה מותנית (Conditional Aggregation)

- **מטרה:** חישוב אחוזים יחסיים של חולים המוגדרים במצב קריטי מתוך סך הכל המאושפזים בכל מחלקה, במטרה להקצות כוח אדם בצורה אופטימלית.
- **טכניקה:** שילוב של `SUM(CASE WHEN...)` יחד עם פונקציות חלוקה ואיחוד קבוצות (`GROUP BY`).
- **צילום הרצה ותוצאה:**
  ![Conditional Aggregation Run](./שלב%203/תמונות/שאילתות/conditional_agg_query.png)

#### 3. דוחות מבצעיים משולבים (Operational Reporting)

- **מטרה:** בניית "דפי קשר" מבצעיים המקשרים אוטומטית בין המטופל המקומי לבין האחראי הלוגיסטי/המפקד הרלוונטי שלו שנמצא בשרת החיצוני.
- **צילום הרצה ותוצאה:**
  ![Operational Report Run](./שלב%203/תמונות/שאילתות/operational_report_query.png)

#### 4. שאילתת אנליזה נוספת - התפלגות חומרת פציעות

- **מטרה:** פילוח סוגי הטראומה והפגיעות הבלנסטיות לפי חטיבות וספקי ציוד רפואי חיצוניים.
- **צילום הרצה ותוצאה:**
  ![Trauma Analysis Run](./שלב%203/תמונות/שאילתות/trauma_analysis_query.png)

#### 5. שאילתת אנליזה נוספת - מדדי יעילות פינוי

- **מטרה:** בדיקת זמני התגובה מרגע הרישום במערכת החיצונית ועד לקליטה הפיזית ומדידת המדדים ב-SmartCare OS.
- **צילום הרצה ותוצאה:**
  ![Efficiency Run](./שלב%203/תמונות/שאילתות/efficiency_query.png)

🔗 **[לחץ כאן לצפייה בקובץ ה-SQL המלא של 5 השאילתות המתוחכמות](./שלב%203/Queries/complex_queries.sql)**

---

_סיום שלב ג' - המערכת מוכנה לאינטגרציה מלאה והרצה בסביבת פרודקשן מבוזרת._
